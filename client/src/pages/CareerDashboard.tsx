    ...
    <div className="w-full pb-32">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-xl">
          <h1 className="text-3xl font-bold text-white mb-2 tracking-wider">CAREER <span className="text-blue-500">INTELLIGENCE</span></h1>
          <p className="text-slate-400">Track your professional journey and interview readiness.</p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-lg">
            <h3 className="text-slate-400 font-semibold mb-1 text-sm uppercase">Job Role</h3>
            <p className="text-2xl font-bold text-blue-400">{profile?.jobRole || 'Undecided'}</p>
          </div>
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-lg">
            <h3 className="text-slate-400 font-semibold mb-1 text-sm uppercase">Resume Status</h3>
            <p className="text-2xl font-bold text-amber-400">{profile?.resumeStatus || 'NEEDS_UPDATE'}</p>
          </div>
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-lg">
            <h3 className="text-slate-400 font-semibold mb-1 text-sm uppercase">DSA Solved</h3>
            <p className="text-2xl font-bold text-green-400">{profile?.dsaProblemsSolved || 0}</p>
          </div>
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-lg">
            <h3 className="text-slate-400 font-semibold mb-1 text-sm uppercase">Projects</h3>
            <p className="text-2xl font-bold text-purple-400">{profile?.projectsCompleted || 0}</p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="bg-slate-800 p-6 rounded-lg border border-slate-700">
          <h2 className="text-xl font-bold text-white mb-4">Career Actions</h2>
          <div className="flex gap-4">
            <input 
              ...
              className="bg-slate-900 border border-slate-600 p-3 rounded text-white w-full max-w-md focus:border-blue-500 focus:outline-none"
            />
            <button 
